...
mdbx.o: $(CORE_SRC) $(CORE_INC) mdbx.h Makefile
...
libmdbx.so: mdbx.o
	$(CC) $(CFLAGS) -save-temps $^ -pthread -shared $(DLSO_LDFLAGS) -o $@

mdbx_%:	src/tools/mdbx_%.c libmdbx.a
	$(CC) $(CFLAGS) $^ $(TEST_LDFLAGS) -o $@

mdbx_test: $(TEST_OBJ) libmdbx.so
	$(CXX) $(CXXFLAGS) $(TEST_OBJ) -Wl,-rpath . -L . -l mdbx $(TEST_LDFLAGS) -o $@
...
cross-gcc:
	...
	@for CC in $(CROSS_LIST_NOQEMU) $(CROSS_LIST); do \
		echo "===================== $$CC"; \
		$(MAKE) clean && CC=$$CC CXX=$$(echo $$CC | sed 's/-gcc/-g++/') TEST_LDFLAGS=-static $(MAKE) all || exit $$?; \
	done
...
cross-qemu:
	...
	@for CC in $(CROSS_LIST); do \
		echo "===================== $$CC + qemu"; \
		$(MAKE) clean && CC=$$CC CXX=$$(echo $$CC | sed 's/-gcc/-g++/') TEST_LDFLAGS=-static $(MAKE) check-singleprocess || exit $$?; \
	done
